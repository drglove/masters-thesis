...
Given that this many muon decays are possible, the muon beam will be placed onto a aluminum target to stop the muons.
...
\begin{figure}[h]
    ...
    \caption{Schematic of a muon decay in the full \mueee detector ready for phase II \cite{Blondel:2013ia}. The cone-shaped aluminum target can be seen in the centre with two $e^+$ tracks curling upward in red, and an $e^-$ track curling down in blue.}
...
\noindent Note that this is process is independent of the target $Z$, since the interaction takes place directly between the proton beam and a single proton on target.
...
During this time, the detector is relatively minimal compared to the full run planned, and currently is using a pixel-only detector.
...
As the protons strike the target, made of lead, zirconium, aluminum alloy, surface muons are created in the aluminum layer.
