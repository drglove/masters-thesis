...
\begin{itemize}
    \item{The total width of the process is given, which we can use to rescale the events. For example, $\Gamma(m_\phi \approx 21\textrm{MeV}) = 1.258\times 10^{-11}~\Gamma(\mu^+ \rightarrow \textrm{all}) \left(g_{\phi\mu}/10^{-4}\right)^2$. \madgraph tallies this partial width for us. This will capture the overall zeroth-order behaviour of our signal, corresponding to the total number of raw signal events all placed into one bin.}
