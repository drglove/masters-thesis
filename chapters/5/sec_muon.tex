...
We also make use of limits placed on the dark photon at the \mueee experiment, to obtain limits in a second fashion.
...
Equating the number of signal events between the two models for the given limiting $\epsilon$ and an unknown limiting $g_{\phi\mu}$, lets us translate the limits, since the number of events detected for sensitivity to be claimed must be the same.
Doing this we find equation \ref{eqn:gphi_limit_echenard}.
...
The resulting limits can be seen in Fig.\ \ref{fig:mu3e_echenard_limits}.

\begin{figure}[h]
    \centering
    \includegraphics[0.5\textwidth]{Figures/limits/mu3e_echenard}
    \caption{Limits on scalar coupling to muon using $\mu^+$ decay at \mueee. Echenard et.\ al.\ provides the dark photon limits \cite{Echenard:2014lma} and should have a more robust background and detector simulation than our earlier results. In blue are the phase I limits, and in red are the phase II limits.}
    \label{fig:mu3e_echenard_limits}
\end{figure}
